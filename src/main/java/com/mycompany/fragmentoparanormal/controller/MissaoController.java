...
public class MissaoController {
    ...
    private void investigar() {
        ...
        // Se atordoado, perde a ação
        if (atordoado) {
            atordoado = false;
            lblEventos.setText("😵 Você ainda está abalado pela presença perturbadora. Ação perdida.");
        ...
        String resultado = InvestigacaoService.investigar(jogador, itensSessao);
        ...
        if (resultado.equals("FRAGMENTO")) {
            if (paginasEncontradasSessao < PAGINAS_NECESSARIAS) {
                ...
                GameState.registrarPagina(indiceMissao, paginasEncontradasSessao);
                jogador.ganharXp(20);
                ...
                if (com.mycompany.fragmentoparanormal.service.MissaoService.campanhaConcluida()) {
                    GameState.setBossDesbloqueado(true);
                    lblEventos.setText(lblEventos.getText()
                        + "\n\n⚠ Campanha concluída! O Boss Final foi desbloqueado!");
                }
                verificarConclusao();
            } else {
            ...
            if (resultado.contains("ATORDOA")) atordoado = true;
            ...
            com.mycompany.fragmentoparanormal.model.Item itemAchado =
                com.mycompany.fragmentoparanormal.service.InvestigacaoService.ARMAS_LOOT[idxItem];
            ...
            jogador.ganharXp(10);
            lblEventos.setText("🔍 Você encontrou uma pista rara! +10 XP.");
    ...
    // Helpers
    // ------------------------------------------------------------------
    /** XP bônus ao concluir cada missão (índice 0–3). */
    private static final int[] XP_CONCLUSAO = { 100, 150, 200, 300 };

    private void verificarConclusao() {
        if (paginasEncontradasSessao >= PAGINAS_NECESSARIAS) {
            if (missao != null) missao.concluir();
            int xpBonus = (indiceMissao >= 0 && indiceMissao < XP_CONCLUSAO.length)
                          ? XP_CONCLUSAO[indiceMissao] : 100;
            jogador.ganharXp(xpBonus);
            lblEventos.setText(lblEventos.getText()
                + "\n\n✔ Missão concluída! +" + xpBonus + " XP de bônus."
                + "\nContinue explorando ou volte ao menu via Fugir.");
            salvarProgresso();
        }
    }
