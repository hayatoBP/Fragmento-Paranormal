...
public class JogadorDAO {
    ...
    private static void salvarItens(int jogadorId, Personagem jogador) {
        // Limpa os itens antigos e reinserir os atuais
        String deleteSql = "DELETE FROM itens_jogador WHERE jogador_id = ?";
        String insertSql = "INSERT INTO itens_jogador (jogador_id, nome, descricao) VALUES (?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    private static void carregarItens(int jogadorId, Personagem p) {
        String sql = "SELECT nome, descricao FROM itens_jogador WHERE jogador_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
